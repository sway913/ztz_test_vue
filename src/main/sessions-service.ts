...
export class SessionsService {
  ...
  public constructor() {
    ...
    */
    ...
    const getDownloadItem = (item: Electron.DownloadItem, id: string): IDownloadItem => ({
    ...
    const downloadsDialog = () =>
      Application.instance.dialogs.getDynamic("downloads-dialog")?.webContentsView?.webContents;
    ...
    session.defaultSession.on("will-download", (event, item, webContents) => {
      const id = makeId(32);
      const window = Application.instance.windows.list.find((x) => x && x.webContents.id === webContents.id);
