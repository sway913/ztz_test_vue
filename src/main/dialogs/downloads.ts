...
import { DIALOG_MARGIN_TOP, DIALOG_MARGIN, DIALOG_TOP } from "@constants/design";

export const showDownloadsDialog = (browserWindow: BrowserWindow, x: number, y: number) => {
  let height = 0;
  ...
  const dialog = Application.instance.dialogs.show({
    ...
    getBounds: () => {
      ...
      dialog.webContentsView.webContents.send(`max-height`, Math.min(maxHeight, height));
