...
import { DIALOG_MIN_HEIGHT, DIALOG_MARGIN_TOP, DIALOG_MARGIN } from "@constants/design";
...
export class SearchDialog extends PersistentDialog {
  ...
  public constructor() {
    ...
    ipcMain.on(`height-${this.id}`, (e, height) => {
      super.rearrange({
        height: this.isPreviewVisible ? Math.max(DIALOG_MIN_HEIGHT, HEIGHT + height) : HEIGHT + height,
      });
    });
