...
import { Menu, nativeImage, NativeImage, MenuItemConstructorOptions } from "electron";
import { IBookmark } from "@interfaces/index";
...
export function createDropdown(appWindow: AppWindow, parentID: string, bookmarks: IBookmark[]): Electron.Menu {
  const folderBookmarks = bookmarks.filter(({ static: staticName, parent }) => !staticName && parent === parentID);
  const template = folderBookmarks.map<MenuItemConstructorOptions>(({ title, url, favicon, isFolder, _id }) => ({
    click: url
      ? () => {
