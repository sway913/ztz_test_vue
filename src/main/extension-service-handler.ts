...
import { extensionMainChannel, ExtensionMainService } from "@common/rpc/extensions";
import { Application } from "./application";
import { URL } from "url";

export class ExtensionServiceHandler implements RpcMainHandler<ExtensionMainService> {
  constructor() {
  ...
  inspectBackgroundPage(e: RpcMainEvent, id: string): void {
    webContents
      .getAllWebContents()
      .find((x) => x.session === Application.instance.sessions.view && new URL(x.getURL()).hostname === id)
