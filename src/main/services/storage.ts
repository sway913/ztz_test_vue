...
export class StorageService {
  ...
  public constructor(settings: Settings) {
    ...
    ipcMain.handle("topsites-get", (e, count) => {
      return this.historyVisited.filter((x) => x.title && x.title !== "").slice(0, count);
    });
  }
  ...
  public async updateBookmark(id: string, change: IBookmark) {
    const index = this.bookmarks.indexOf(this.bookmarks.find((x) => x._id === id));
  ...
  public addFavicon = async (url: string): Promise<string> => {
    try {
      if (!this.favicons.get(url)) {
        ...
        const str = `data:${(await fileTypeFromBuffer(data))?.ext};base64,${data.toString("base64")}`;
  ...
  private createBookmarkArray = (parentFolderId: string = null, first = true, depth = 1): string[] => {
    let payload: string[] = [];
    let title;
    const bookmarks = this.bookmarks.filter((x) => x.parent === parentFolderId);
    const indentFirst = indentType.repeat(depth * indentLength);
    const indentNext = !first ? indentFirst : indentType.repeat((depth + 1) * indentLength);
    ...
    for (const bookmark of bookmarks) {
      if (!bookmark.isFolder && bookmark.url) {
        ...
        payload.push(`${indentNext}<DT><A HREF="${href}" ICON="${icon}">${title}</A>`);
      } else if (bookmark.isFolder) {
        title = encodeTitle(bookmark.title);
        payload.push(`${indentNext}<DT><H3>${title}</H3>`);
        payload = payload.concat(this.createBookmarkArray(bookmark._id, true, depth + 1));
      }
    }
  ...
  public exportBookmarks = async () => {
    ...
    const bar = this.createBookmarkArray(this.bookmarks.find((x) => x.static === "main")._id);

    const other = this.createBookmarkArray(this.bookmarks.find((x) => x.static === "other")._id, false);
