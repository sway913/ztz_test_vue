...
export class DialogsService {
  ...
  public show(options: IDialogShowOptions): IDialog {
    const { name, browserWindow, getBounds, devtools, onHide, hideTimeout, onWindowBoundsUpdate, tabAssociation } =
      options;
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      webContentsView.webContents.loadURL(join("file:///", app.getAppPath(), `build/${name}.html`));
    }
  ...
  public sendToAll = (channel: string, ...args: any[]) => {
    this.getChildViews().forEach((x) => !x.webContents.isDestroyed() && x.webContents.send(channel, ...args));
  };
