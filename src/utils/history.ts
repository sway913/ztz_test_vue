import { IHistoryItem, IVisitedItem } from "@interfaces/index";
...
export const countVisitedTimes = (hItems: IHistoryItem[]): IVisitedItem[] => {
  ...
  for (const historyItem of historyItems) {
    const itemsWithUrl = historyItems.filter((x) => x.url.replace(/\//g, "") === historyItem.url.replace(/\//g, ""));
    ...
    if (!items.find((x) => x.url.replace(/\//g, "") === historyItem.url.replace(/\//g, ""))) {
